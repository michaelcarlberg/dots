...
# {
#   log::defer "Check firewall status"
#
#   if sudo ufw status | grep -q inactive; then
#     log::defer::failure "" "Command not listed in commands.json"
#     exit 1
#   else
#     log::defer::success
#   fi
...
#   log::defer::success
#
#   log "Enter the 4-digit code shown on the TV: $(ansi::save-position)"
#   ansi::restore-position
#   read -r auth_code
#
#   auth_header="Authorization: Basic $(printf ":%s" "$auth_code" | base64)"
#
#   log::defer "Trying to authenticate using $(ansi::bold "$auth-code")"
#
...
function main
{
  local tunnelname='vpn'
  local connection='wlan0'

  while [[ ${1:0:1} == "-" ]]; do
    case "$1" in
      "-h" | "--help")
        cli::usage_from_commentblock
        exit
        ;;
      "-t"* | "--tunnel"*) tunnelname=$(cli::get_argument_value "$@") && shift ;;
